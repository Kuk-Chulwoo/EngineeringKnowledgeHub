...
from backend.app.migration_v4 import migrate_v3_to_v4
...
def test_v4_to_v5_preserves_company_parts_pins_and_documents(tmp_path):
    path = tmp_path / "hub.sqlite3"
    before = populated_v1(path)
    with sqlite3.connect(path) as connection:
        connection.execute("BEGIN IMMEDIATE")
        ...
        connection.execute(
            ...
            (component_id,pin_number,pin_name,source_type,created_at,updated_at)
            VALUES (1,'EP','GND','MANUAL','now','now')"""
        )
    Database(path).initialize()
    assert rows(path) == before
    with sqlite3.connect(path) as connection:
        ...
        assert connection.execute(
            "SELECT pin_number,pin_name FROM component_pins"
        ).fetchall() == [("EP", "GND")]
        assert connection.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='schematic_symbols'"
        ).fetchone()
    assert len(list(tmp_path.glob("*.v4-backup-*"))) == 1
